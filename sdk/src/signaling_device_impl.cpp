...
SignalingDeviceImplPtr SignalingDeviceImpl::create(const SignalingDeviceConfig& conf) {
    return std::make_shared<SignalingDeviceImpl>(conf);
}

SignalingDeviceImpl::SignalingDeviceImpl(const SignalingDeviceConfig& conf) : wsImpl_(conf.wsImpl), httpCli_(conf.httpCli), ws_(WebsocketConnection::create(wsImpl_, conf.timerFactory)), timerFactory_(conf.timerFactory), deviceId_(conf.deviceId), productId_(conf.productId), tokenProvider_(conf.tokenProvider), httpHost_(conf.signalingUrl) {
    if (httpHost_.empty()) {
...
void SignalingDeviceImpl::doConnect() {
    ...
    req.body = nlohmann::json({{"deviceId", deviceId_}, {"productId", productId_}}).dump();

    auto self = shared_from_this();
    httpCli_->sendRequest(method, url, req, [self](const std::unique_ptr<SignalingHttpResponse>& response) {
        const int httpOkStartRange = 200;
        const int httpOkEndRange = 299;
        if (response == nullptr) {
            NABTO_SIGNALING_LOGE << "HTTP request failed ";
            self->waitReconnect();
        } else if (response->statusCode < httpOkStartRange || response->statusCode > httpOkEndRange) {
            NABTO_SIGNALING_LOGE << "HTTP request failed with status: " << response->statusCode;
            self->waitReconnect();
        } else {
...
void SignalingDeviceImpl::handleWsMessage(SignalingMessageType type, const nlohmann::json& message) {
    ...
    try {
        ...
        try {
            ...
            NABTO_SIGNALING_LOGD << "Got websocket channel for unknown channel ID: " << connId;
            // if message type is MESSAGE, we will create a connection below, otherwise we ignore the message
        }
        ...
        NABTO_SIGNALING_LOGE << "Invalid channel ID in websocket message: " << message.dump() << " error: " << exception.what();
    }
}
...
void SignalingDeviceImpl::parseAttachResponse(const std::string& response) {
    try {
        ...
        NABTO_SIGNALING_LOGE << "Failed parse attach response: " << exception.what();
    }
}
...
void SignalingDeviceImpl::waitReconnect() {
    ...
    if (reconnectCounter_ < exponentialBackoffCap) {
        reconnectWait = msPrSecond * (static_cast<uint32_t>(1) << reconnectCounter_);
    }
    reconnectCounter_++;
...
void SignalingDeviceImpl::requestIceServers(IceServersResponse callback) {
    ...
    auto self = shared_from_this();
    httpCli_->sendRequest(method, url, req, [self, callback](const std::unique_ptr<SignalingHttpResponse>& response) {
        if (response == nullptr) {
            callback({});
        } else {
            const int httpOkStartRange = 200;
            const int httpOkEndRange = 299;
            if (response->statusCode >= httpOkStartRange && response->statusCode <= httpOkEndRange) {
...
std::vector<struct IceServer> SignalingDeviceImpl::parseIceServers(const std::string& data) {
    ...
    try {
        ...
        for (auto server : servers) {
            ...
            const struct IceServer iceServer = {
                username,
                cred,
                iceUrls};
        ...
        NABTO_SIGNALING_LOGE << "Failed to parse ICE servers with: " << exception.what() << " Parsing data: " << data;
    }
    return result;
}
