 ...
 * failed.
 */
using HttpResponseCallback = std::function<void(std::unique_ptr<SignalingHttpResponse> response)>;
...
class SignalingHttpClient {
   public:
     ...
     */
    virtual bool sendRequest(const std::string& method, const std::string& url, const SignalingHttpRequest& request, HttpResponseCallback callback) = 0;
};
...
class SignalingWebsocket {
   public:
     ...
     * @param callback the callback to set
     */
    virtual void onMessage(std::function<void(const std::string& message)> callback) = 0;
...
class SignalingTimer {
   public:
     ...
     */
    virtual void setTimeout(uint32_t timeoutMs, std::function<void()> callback) = 0;
};
...
struct IceServer {
    /**
     * username will be the empty string if the server is a STUN server, and a username if it is a TURN server
     */
    std::string username;
    /**
     * credential will be the empty string if the server is a STUN server, and a credential if it is a TURN server
     */
    std::string credential;
    /**
     * List of URLs for the ICE server. If the server is a TURN server, the credentials will be valid for all URLs in the list.
     */
 ...
 * tokenProvider is a function the Signaler calls when it requires a token allowing the Signaler to attach to the Nabto Backend
 * signalingUrl is the URL for the Nabto Backend eg. https://signalingpoc.dev.nabto.com
 * wsImpl is an instance of the Websocket abstraction defined above
 * httpCli is an instance of the HTTP client abstraction defined above
 * timerFactory is an instance of the Timer factory abstraction defined above
 */
struct SignalingDeviceConfig {
    ...
    /**
     * WebSocket implementation the SDK can use to send and receive signaling messages.
     */
    SignalingWebsocketPtr wsImpl;

    /**
     * HTTP client implementation the SDK can use to send HTTP requests and receive responses.
     */
...
class SignalingChannel {
   public:
    ...
    /**
     * Set a handler to be invoked whenever a message is available on the connection
     *
     * @param handler the handler to set
     */
