...
bool CurlHttpClient::sendRequest(const std::string& method, const std::string& url, const nabto::signaling::SignalingHttpRequest& request, nabto::signaling::HttpResponseCallback cb) {
    ...
    if ((res = curl_easy_setopt(curl, CURLOPT_URL, url.c_str())) != CURLE_OK ||
        ...
        (res = curl_easy_setopt(curl, CURLOPT_WRITEDATA, (void*)&writeBuffer_)) != CURLE_OK ||
        (res = curl_easy_setopt(curl, CURLOPT_READFUNCTION, readFunc)) != CURLE_OK ||
        (res = curl_easy_setopt(curl, CURLOPT_READDATA, (void*)&readBuffer_)) != CURLE_OK) {
        std::cout << "Failed to initialize Curl request with CURLE: " << curl_easy_strerror(res) << std::endl;
        return false;
    }

    if (method == "POST" && (res = curl_easy_setopt(curl, CURLOPT_POST, 1L)) != CURLE_OK) {
        std::cout << "Failed to initialize Curl request with CURLE: " << curl_easy_strerror(res) << std::endl;
        return false;
    }
    ...
    curl_->asyncInvoke([self, cb, curl](CURLcode res, uint16_t statusCode) {
        // std::cout << "Got curl response code: " << res << " statusCode: " << statusCode << std::endl;
        if (res != CURLE_OK) {
...
size_t CurlHttpClient::readFunc(void* ptr, size_t size, size_t nmemb, void* s) {
    try {
        ...
        // std::cout << "size: " << size << " nmemb: " << nmemb << " buf size: " << buf->size();
        // std::cout << "Copying " << ncpy << "bytes from: " << buf;
...
size_t CurlHttpClient::writeFunc(void* ptr, size_t size, size_t nmemb, void* s) {
    if (s == stdout) {
...
bool CurlAsync::init() {
    ...
    if (res != CURLE_OK) {
        std::cout << "Failed to initialize Curl global with: " << curl_easy_strerror(res) << std::endl;
        return false;
    }
    curl_ = curl_easy_init();
    if (!curl_) {
        std::cout << "Failed to initialize Curl easy with: " << curl_easy_strerror(res) << std::endl;
        return false;
    }

    res = curl_easy_setopt(curl_, CURLOPT_VERBOSE, 0L);
    if (res != CURLE_OK) {
        std::cout << "Failed to set curl logging option with: " << curl_easy_strerror(res) << std::endl;
        return false;
    }
    res = curl_easy_setopt(curl_, CURLOPT_NOPROGRESS, 1L);
    if (res != CURLE_OK) {
        std::cout << "Failed to set Curl progress option with: " << curl_easy_strerror(res) << std::endl;
        return false;
    }
    return true;
}
