...
class CurlHttpClient : public nabto::signaling::SignalingHttpClient, public std::enable_shared_from_this<CurlHttpClient> {
   public:
    static nabto::signaling::SignalingHttpClientPtr create() {
        return std::make_shared<CurlHttpClient>();
    }
    CurlHttpClient();
    bool sendRequest(const std::string& method, const std::string& url, const nabto::signaling::SignalingHttpRequest& request, nabto::signaling::HttpResponseCallback cb);
