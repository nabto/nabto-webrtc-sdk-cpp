...
public:
    ...
    static std::shared_ptr<TestInstance> create(bool extraRespData, bool failHttp, bool failWs) {
    ...
    bool connectClient(std::string cliId) {
        try {
            auto resp =  api_->postTestDeviceByTestIdClientsByClientIdConnect(testId_, cliId).get();
            return true;
        } catch (api::ApiException& ex) {
    ...
    bool clientSendMessage(std::string& cliId, std::string message)
    {
        ...
        auto req = std::make_shared <model::PostTestClientByTestIdSend_device_messages_request>();
        req->setMessages(msgs);

        try {
            auto resp = api_->postTestDeviceByTestIdClientsByClientIdSendMessages(testId_, cliId, req).get();
            return true;
        } catch (api::ApiException& ex) {
    ...
    bool disconnectClient(std::string& cliId) {
        try {
            auto resp = api_->postTestDeviceByTestIdClientsByClientIdDisconnect(testId_, cliId).get();
            return true;
        } catch (api::ApiException& ex) {
    ...
    bool sendNewMessageType() {
        ...
        try {
            ...
            std::cout << "Failed to send new message type: " << ex.what() << std::endl;
            // if (resp) {
            //     auto cBody = object_convertToJSON(resp);
            //     std::string err = std::string("HTTP Failed: ") + cJSON_Print(cBody);
            //     cJSON_Delete(cBody);
    ...
    std::vector<std::string> clientWaitForMessagesIsReceived(std::string cliId, std::vector<std::string>& msgs) {

        auto req = std::make_shared < model::PostTestClientByTestIdWait_for_device_messages_request>();
        req->setTimeout(200);
        req->setMessages(msgs);

        try {
            auto resp = api_->postTestDeviceByTestIdClientsByClientIdWaitForMessages(testId_, cliId, req).get();
    ...
    ProtocolError clientWaitForErrorReceived(std::string cliId) {
        ...
        auto req = std::make_shared < model::PostTestDeviceByTestIdClientsByClientIdWait_for_error_request>();
        req->setTimeout(200);

        try {
            auto resp = api_->postTestDeviceByTestIdClientsByClientIdWaitForError(testId_, cliId, req).get();
    ...
    bool clientSendError(std::string cliId, std::string errorCode, std::string errorMessage) {
        auto req = std::make_shared < model::PostTestClientByTestIdSend_device_error_request>();
        req->setErrorCode(errorCode);
        req->setErrorMessage(errorMessage);

        try {
            auto resp = api_->postTestDeviceByTestIdClientsByClientIdSendError(testId_, cliId, req).get();
            return true;
        } catch (api::ApiException& ex) {
            std::cout << "Failed to send new message type: " << ex.what() << std::endl;
            throw std::runtime_error("HTTP Failed");
    ...
    nabto::signaling::SignalingDevicePtr getDevice() {
        return sig_;
    }
