...
static bool sha256hex(const std::vector<uint8_t> &data, std::string &hash) {
    std::vector<uint8_t> outputBuffer = std::vector<uint8_t>(EVP_MD_size(EVP_sha256()));
    ...
    for (i = 0; i < outputBuffer.size(); i++) {
        hexOutput << std::setfill('0') << std::setw(2) << std::hex << static_cast<int>(outputBuffer[i]);
    }
...
static bool getSpkiFromPrivateKey(const std::string &privateKey, std::vector<uint8_t> &spki) {
    BIOPtr bio(BIO_new_mem_buf((void *)privateKey.c_str(), -1));  // -1 = null-terminated string
    if (!bio) {
...
static bool getKeyIdFromPrivateKey(const std::string &privateKey, std::string &keyId) {
