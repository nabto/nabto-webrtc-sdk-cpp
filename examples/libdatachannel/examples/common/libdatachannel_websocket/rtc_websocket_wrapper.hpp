...
class RtcWebsocketWrapper : public nabto::signaling::SignalingWebsocket, public std::enable_shared_from_this<RtcWebsocketWrapper> {
   public:
    ...
    void onMessage(std::function<void(const std::string& message)> callback) {
        ...
        ws_.onMessage([self, callback](std::variant<rtc::binary, rtc::string> message) {
            std::string msg;
