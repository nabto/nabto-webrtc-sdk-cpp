...
bool CurlHttpClient::sendRequest(const std::string& method, const std::string& url, const nabto::signaling::SignalingHttpRequest& request, nabto::signaling::HttpResponseCallback cb) {
    ...
    if ((res = curl_easy_setopt(curl, CURLOPT_URL, url.c_str())) != CURLE_OK ||
        ...
        NPLOGE << "Failed to initialize Curl request with CURLE: " << curl_easy_strerror(res);
        return false;
    }
    ...
    curl_->asyncInvoke([self, cb, curl](CURLcode res, uint16_t statusCode) {
        NPLOGI << "Got curl response code: " << res << " statusCode: " << statusCode << std::endl;
        if (res != CURLE_OK) {
...
size_t CurlHttpClient::readFunc(void* ptr, size_t size, size_t nmemb, void* s) {
    try {
        ...
        // NPLOGI << "size: " << size << " nmemb: " << nmemb << " buf size: " << buf->size();
        // NPLOGI << "Copying " << ncpy << "bytes from: " << buf;
