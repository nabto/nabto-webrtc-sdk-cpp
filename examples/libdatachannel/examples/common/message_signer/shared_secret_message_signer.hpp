   ...
   public:
    ...
    nlohmann::json signMessage(const nlohmann::json& msg) override {
        if (nextMessageSignSeq_ != 0 && remoteNonce_.empty()) {
            NPLOGE << "Tried to sign message with seq: " << nextMessageSignSeq_ << " and an empty remote nonce";
            throw std::runtime_error("Invalid State");
        }
        NPLOGD << "Signing message with seq: " << nextMessageSignSeq_ << " and remoteNonce: " << remoteNonce_;
    ...
    nlohmann::json verifyMessage(const nlohmann::json& msg) override {
        ...
        try {
            ...
            // TODO: handle optional key ID and find secret based on key ID if it exists
