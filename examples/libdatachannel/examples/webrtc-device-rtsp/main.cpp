...
int main(int argc, char** argv) {
    ...
    plog::init<nabto::signaling::SIGNALING_LOGGER_INSTANCE_ID>(opts.logLevel, &consoleAppender);

    NPLOGI << "Connecting to device: " << opts.deviceId;

    nabto::example::NabtoJwtPtr jwtPtr = nabto::example::NabtoJwt::create(opts.productId, opts.deviceId, opts.privateKey);
...
bool parse_options(int argc, char** argv, struct options& opts)
{
    try
    {
        ...
        options.add_options()
            ("d,deviceid", "Device ID for device", cxxopts::value<std::string>())
            ("p,productid", "Product ID for device", cxxopts::value<std::string>())
            ("log-level", "Optional. The log level (error|info|trace)", cxxopts::value<std::string>())
            ("k,private-key-file", "Optional. path to pem private key file to use", cxxopts::value<std::string>())
            ("s,signaling-url", "Optional. URL for the signaling service", cxxopts::value<std::string>())
            ("secret", "Optional. Shared secret used to sign and validate signaling messages", cxxopts::value<std::string>())
            ("central-authorization", "Require central authorization")
        ...
        if (result.count("log-level"))
        {
            ...
            if (lvl == "error") {
                ...
                std::cout << "Invalid log level provided: " << lvl << " using default" << std::endl;
            }
        }

        if (result.count("private-key-file"))
        {
            opts.privateKey = readKeyFile(result["private-key-file"].as<std::string>());
        }
        ...
        if (opts.sharedSecret.empty() && opts.centralAuthorization == false) {
            std::cout << "Error atleast one of the options --secret or --central-authorization needs to be enabled" << std::endl;
            return false;
        }
