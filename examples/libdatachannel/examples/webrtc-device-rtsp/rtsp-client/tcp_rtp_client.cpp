...
void TcpRtpClient::setConnection(std::shared_ptr<rtc::Track> videoTrack, std::shared_ptr<rtc::Track> audioTrack)
{
    ...
    if (videoTrack != nullptr) {
        ...
        videoRepacketizer_ = videoRepack_->createPacketizer(videoTrack_, videoSsrc_, pt);
        videoDstPt_ = pt;
    }
    if (audioTrack != nullptr) {
        ...
        auto pts = desc.payloadTypes();
        int pt = pts.empty() ? 0 : pts[0];
        audioRepacketizer_ = audioRepack_->createPacketizer(audioTrack_, audioSsrc_, pt);
        audioDstPt_ = pt;
    }
}
    ...
    if (channel == 0) {
        ...
        if (self->videoTrack_ != nullptr && self->videoTrack_->isOpen()) {
            ...
            try {
                ...
                // This was introduced as we observed a runtime error due to the track being closed. Since we check for isOpen(), this appears to be a race condition. Both us and libdatachannel have mutex protection, so this will not be a memory issue. However, libdatachannel can still change the Open state between the check above and this.
