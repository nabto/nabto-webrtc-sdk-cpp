...
class RtcpClient : public std::enable_shared_from_this<RtcpClient> {
 public:
  ...
  RtcpClient(uint16_t port) : port_(port) {}
  ...
  void start() {
    ...
    if (bind(rtcpSock_, reinterpret_cast<const sockaddr*>(&addr),
             sizeof(addr)) < 0) {
    ...
    setsockopt(rtcpSock_, SOL_SOCKET, SO_RCVBUF,
 ...
 private:
  static void rtcpRunner(RtcpClient* self) {
    ...
    while ((len = recvfrom(self->rtcpSock_, buffer, RTP_BUFFER_SIZE, 0,
      ...
      auto ret = sendto(self->rtcpSock_, rr, rr->header.lengthInBytes(), 0,
