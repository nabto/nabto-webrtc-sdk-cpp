...
size_t RtspClient::addConnection(std::shared_ptr<rtc::Track> videoTrack, std::shared_ptr<rtc::Track> audioTrack)
{
    ...
    if (started_) {
        ...
        if (videoStream_ != nullptr && videoTrack != nullptr) {
            rtsp.videoRef = videoStream_->addConnection(videoTrack, videoSsrc_, videoPayloadType_);
        }
        if (audioStream_ != nullptr && audioTrack != nullptr) {
            rtsp.audioRef = audioStream_->addConnection(audioTrack, audioSsrc_, audioPayloadType_);
        }
    } else {
        auto self = shared_from_this();
        start([self, &rtsp, videoTrack, audioTrack](std::optional<std::string> error) {
            if (error.has_value()) {
            ...
            if (self->videoStream_ != nullptr && rtsp.videoTrack != nullptr) {
                rtsp.videoRef = self->videoStream_->addConnection(rtsp.videoTrack, self->videoSsrc_, self->videoPayloadType_);
            }
            if (self->audioStream_ != nullptr && rtsp.audioTrack != nullptr) {
                rtsp.audioRef = self->audioStream_->addConnection(rtsp.audioTrack, self->audioSsrc_, self->audioPayloadType_);
            }
        });
    }
...
void RtspClient::setupRtsp() {
    ...
    if (!videoControlUrl_.empty()) {
        ...
        if (preferTcp_) {
            if (tcpClient_ == nullptr) {
                TcpRtpClientConf conf = { curl_, sessionControlUrl_, videoRepack_, audioRepack_, videoPayloadType_, audioPayloadType_, videoSsrc_, audioSsrc_};
    ...
    if (!audioControlUrl_.empty()) {
        ...
        if (preferTcp_) {
            ...
            nabto::example::RtpClientConf conf = { std::string(), (uint16_t)(port_ + 2) };
