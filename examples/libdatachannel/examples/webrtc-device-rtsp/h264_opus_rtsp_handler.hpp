...
class H264TrackHandler : public WebrtcTrackHandler, public  std::enable_shared_from_this<H264TrackHandler> {
   public:
    static WebrtcTrackHandlerPtr create(std::string rtspUrl) {
        return std::make_shared<H264TrackHandler>(rtspUrl);
    }

    H264TrackHandler(std::string rtspUrl) : rtspUrl_(rtspUrl), ssrc_(SsrcGenerator::generateSsrc()), audioSsrc_(SsrcGenerator::generateSsrc()) {
        RtspClientConf conf = { rtspUrl, videoRepack_, nullptr, 96, 111, ssrc_, audioSsrc_ };
        rtp_ = RtspClient::create(conf);
    }

    size_t addTrack(std::shared_ptr<rtc::PeerConnection> pc) {
        counter_++;
        RtspClientConf conf = { rtspUrl_, videoRepack_, nullptr, 96, 111, ssrc_, audioSsrc_, true, (uint16_t)(42222 + (counter_ * 4)) };
        HandlerTrack track;
