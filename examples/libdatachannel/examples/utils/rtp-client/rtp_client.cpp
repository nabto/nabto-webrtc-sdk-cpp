...
RtpClient::RtpClient(const RtpClientConf& conf) : remoteHost_(conf.remoteHost),
...
size_t RtpClient::addConnection(std::shared_ptr<rtc::Track> track, const rtc::SSRC ssrc, int payloadType) {
...
void RtpClient::addConnection(RtpTrack track) {
    ...
    track.track->onMessage([self, track](rtc::message_variant data) {
        ...
        if (msg->type == rtc::Message::Binary) {
            ...
            auto ret = sendto(self->videoRtpSock_, msg->data(), msg->size(), 0, (struct sockaddr*)&addr, sizeof(addr));
        }
    });
}
...
void RtpClient::rtpVideoRunner(RtpClient* self) {
    ...
    while (true) {
        ...
        {
            std::lock_guard<std::mutex> lock(self->mutex_);
            for (auto it = self->mediaTracks_.begin(); it != self->mediaTracks_.end(); ++it) {
                if (it->track->isOpen()) {
                    try {
